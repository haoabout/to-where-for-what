...
The guide also lays itself out on **A4 sheets** — one section per sheet, the
same type size on every one, page numbers in the footer. Print from the browser
and you have the PDF. A `---` in `route.md` is where a new sheet starts.
...
- **Map**: OSM vector basemaps (two independent sources backing each other up)
  ...
  corner button — the page around it is a fixed deep-navy canvas with light
  paper surfaces, and has no theme switch of its own
- **Weather**: fetched live from Open-Meteo in the browser. A real forecast
  ...
  clearly labeled
- **Export**: CSV (imports straight into Google My Maps) and KML, plus the
  guide as A4 pages — the browser's print dialog produces the PDF
...
攻略还能按 **A4 分页**排：一节一页，每页字号一致，页脚带页码，浏览器打印出来就是 PDF。`route.md` 里的 `---` 就是分页处。

## 页面里有什么

- **地图**：OSM 矢量底图（两个独立数据源互为备份）+ 从 OSM 抓的**地铁线路与车站，用官方线路配色**。两个矢量底图各有明暗两版，由地图右上角的开关切换；页面本身是固定的深蓝底 + 浅色纸面，没有自己的明暗切换
- **天气**：浏览器端实时拉 Open-Meteo。16 天内是预报，超出则退回过去 8 年同期均值并明确标注
- **导出**：CSV（可直接导入 Google My Maps）、KML；攻略可按 A4 分页，浏览器打印即得 PDF
