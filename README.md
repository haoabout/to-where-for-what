...
windows, caveats. The cost summary, the pre-departure to-dos and the all-places
table are generated by the page from the data, so they always match your
choices. The to-dos list every visit that needs booking (one row per visit) and
anything left unverified; ticking a box writes back into the data, the printed
guide doubles as a paper checklist, and the validator warns when departure is
near with must-book visits still unticked.
