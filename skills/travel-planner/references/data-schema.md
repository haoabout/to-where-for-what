...
| `origin` | `user` | | 出处标记。`user` = 用户在行程页地图上搜索（Nominatim）手动添加的点，见下方「用户粗胚」。**AI 补全后保留此字段**——它记录的是这个点从哪来，不是补全了没有 |
...
用户在行程页的地图上临时搜索添加的点。页面端只有搜索结果给的信息
（名称 + 坐标 + OSM 链接），研究型字段一个都没有——**这是刻意的**：
逼页面造 hours/tier 只会得到假数据。分工是「用户加粗胚、AI 事后补全」。

- id 前缀 `u-`（页面用时间戳 base36 生成，如 `u-mjq3k8x1`），与 AI 的
  `<城市缩写>-<序号>` 序列天然不冲突。
- **粗胚的判定是 `origin=="user"` 且无 `tier`**。`origin` 是永久的出处标记，
  补全与否看 `tier` 在不在。
- 未补全时走精简必填集（校验器同此）：

| 必填 | 豁免 |
|---|---|
| `id` `name` `coord` `sources` | 其余全部（`sources` 填 OSM 条目链接——名称和坐标确实来源于它） |

- **坐标出 `trip.bbox` 对粗胚只报 P1 不报 P0**：临时起意的点常在 bbox 边缘外
  （大阪行程想去奈良），且坐标来自 OSM 而非 AI 之手，写反/搜错的先验低得多。
- AI 在已有行程上继续工作时，应把「`origin=="user"` 且无 `tier`」的点当作
  待研究清单走研究管线补全（见 SKILL.md「用户粗胚补全」）。补全后按普通
  景点的完整必填集校验。
...
用户在排程视图里把地点分到各天、排出顺序，结果写在这里。

**A 阶段不要预填这个字段。** 页面打开时若 `itinerary` 为空，会按 `trip.dates`
自动逐日建好空容器（日期、周几都算好）。两处都造同一份数据迟早会对不上，
所以生成规则只留在页面里一处。`trip.dates` 缺失时退回按 `trip.days` 建 N 天、
`date` 留空——那时闭馆冲突校验做不了，但容器仍然可用。
...
4. **`n` 可以是 0。** 「第 0 天（抵达当晚）」是常见形态——航班傍晚落地、
   当晚只够在住处附近走走。页面上由用户手动添加，日期取 `dates.start` 的前一天。
