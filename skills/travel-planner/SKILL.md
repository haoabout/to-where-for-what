...
| 默认底图画不出地铁线路 | OpenFreeMap 有地铁几何和双语站名，但没有线路名和配色。切到「地铁」底图（ÖPNVKarte）可以看到线号，但那是张独立专题图，不是叠加层——切过去就看不到原来的底图了 |
| 超过 16 天的天气不是预报 | 自动退回历史同期均值，界面已标注，但你也要说一句 |
| 服务停止后功能降级 | `file://` 打开仍可读，但不能直写文件、不能用 OSM 和地铁底图（后者要靠本地服务代理绕开 CORS） |
